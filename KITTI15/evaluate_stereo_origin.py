...
def validate_eth3d(model, iters=32, mixed_prec=False):
    ...
    for val_id in range(len(val_dataset)):
        ...
        fil = os.path.join('/home/zhengdian/code/IGEV-Stereo', imageL_file.split('/')[-4])
        fil = os.path.join(fil, imageL_file.split('/')[-3])
        fil = os.path.join(fil, imageL_file.split('/')[-2])
        os.makedirs(fil, exist_ok=True)
        jfil = os.path.join(fil, imageL_file.split('/')[-1])
        ...
        disp = flow_pr.numpy().squeeze(0)
        # disp = (disp - disp.min()) / (disp.max() - disp.min())
        disp = disp / 64
        disp = np.round(disp * 256).astype(np.uint16)
        plt.imsave(jfil, disp, cmap='jet')
        # skimage.io.imsave(jfil, disp)
...
def validate_kitti(model, iters=32, mixed_prec=False):
    ...
    for val_id in range(len(val_dataset)):
        ...
        epe_flattened = epe.flatten()
        val = (valid_gt.flatten() >= 0.5) #& (flow_gt.abs().flatten() < 1000)
        # val = valid_gt.flatten() >= 0.5
