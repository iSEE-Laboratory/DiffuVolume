...
def train(args):
    ...
    model = nn.DataParallel(IGEVStereo_ddim(args))
    ...
    if args.restore_ckpt is not None:
        ...
        pre_dict = {k: v for k, v in state_dict.items() if k in model_dict}
    ...
    while should_keep_training:
        ...
        for i_batch, (_, *data_blob) in enumerate(tqdm(train_loader)):
            ...
            flow = torch.clamp(flow, 0, 192 - 1)
            flow_4 = F.interpolate(flow, size=(h // 4, w // 4), mode='bilinear') / 4

            assert model.training
            disp_init_pred, disp_preds = model(image1, image2, flow, flow_4, iters=args.train_iters)
            assert model.training

            loss, metrics = sequence_loss(disp_preds, disp_init_pred, flow, valid, max_disp=args.max_disp)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', default='./checkpoints/kitti', help='the directory to save logs and checkpoints')
