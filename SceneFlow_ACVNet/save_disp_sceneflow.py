...
parser.add_argument('--datapath', default="/home/zhengdian/dataset/Sceneflow/", help='data path')
parser.add_argument('--testlist', default='./filenames/test_temp.txt', help='testing list')
parser.add_argument('--loadckpt', default='/home/zhengdian/code/ACVNet-main/checkpoints/checkpoint_000046.ckpt')
# parse arguments
...
state_dict = torch.load('/home/zhengdian/code/ACVNet-main/pretrained_model/sceneflow.ckpt')
model_origin.load_state_dict(state_dict['model'])

save_dir = '/home/zhengdian/code/ACVNet-main/temp_c/'
...
def test():
    ...
    for batch_idx, sample in enumerate(TestImgLoader):
        ...
        for disp_est, top_pad, right_pad, fn in zip(disp_est_np, top_pad_np, right_pad_np, left_filenames):
            ...
            fil = os.path.join(save_dir, fn.split('/')[-4])
            fil = os.path.join(fil, fn.split('/')[-3])
            os.makedirs(fil, exist_ok=True)
            fil = os.path.join(fil, fn.split('/')[-1])
            print("saving to", fil, disp_est.shape)
            disp_est_uint = np.round(disp_est * 255).astype(np.uint16)
            #skimage.io.imsave(fil, disp_est_uint)
            plt.imsave(fil, disp_est_uint, cmap='jet')
            #cv2.imwrite(fn,disp_est_uint, )
            # cv2.imwrite(fn, cv2.applyColorMap(cv2.convertScaleAbs(disp_est_uint, alpha=0.008), cv2.COLORMAP_JET))
