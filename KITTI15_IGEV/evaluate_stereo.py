...
@torch.no_grad()
def validate_kitti(model_origin, model, iters=32, mixed_prec=False):
    ...
    for val_id in range(len(val_dataset)):
        ...
        flow_refine = padder.unpad(flow_refine.unsqueeze(1)).cpu().squeeze(0)
        assert flow_refine.shape == flow_gt.shape, (flow_refine.shape, flow_gt.shape)
        
        epe = torch.sum((flow_refine - flow_gt)**2, dim=0).sqrt()
...
def validate_sceneflow(model, iters=32, mixed_prec=False):
    ...
    for val_id in tqdm(range(len(val_dataset))):
        ...
        # epe = torch.sum((flow_pr - flow_gt)**2, dim=0).sqrt()
        epe = torch.abs(flow_refine - flow_gt)
...
def validate_middlebury(model, iters=32, split='F', mixed_prec=False):
    ...
    for val_id in range(len(val_dataset)):
        ...
        val = (valid_gt.reshape(-1) >= 0.5) & (flow_gt[0].reshape(-1) < 1000)# & (occ_mask==255)
...
if __name__ == '__main__':
    ...
    if args.restore_ckpt is not None:
        ...
        checkpoint = torch.load('./pretrained_models/kitti/kitti15.pth')  #modify
        model_origin.load_state_dict(checkpoint, strict=True)
    ...
    if args.dataset == 'eth3d':
        ...
        validate_kitti(model_origin, model, iters=args.valid_iters, mixed_prec=use_mixed_precision)
